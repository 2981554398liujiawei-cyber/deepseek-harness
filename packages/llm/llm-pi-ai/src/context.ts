 ...
 * @param degradeImages - replace images with durable text references instead of base64 content.
 * @param onReplayDegrade - forwarded to {@link toPiAssistant} for each assistant message.
...
export function toPiContext(
  options: GenerateOptions,
  attachments?: AttachmentStore,
  degradeImagesOrReplay?: boolean | ((reason: string) => void),
  onReplayDegrade?: (reason: string) => void,
): PiContext | Promise<PiContext> {
  const degradeImages = typeof degradeImagesOrReplay === 'boolean' ? degradeImagesOrReplay : false
  const replayDegrade = typeof degradeImagesOrReplay === 'function' ? degradeImagesOrReplay : onReplayDegrade
  return attachments === undefined && !degradeImages
    ? textOnlyContext(options, replayDegrade)
    : toPiContextWithImages(options, attachments, degradeImages, replayDegrade)
}
