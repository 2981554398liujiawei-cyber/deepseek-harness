/**
 * Translate RightAPI SSE payloads with one stateful harness block per content, reasoning, or tool
 ...
 *
 * Translate RightAPI wire chunks into the harness `StreamChunk` protocol.
 * @module dsh-llm-rightapi/translate
 */
 ...
 * @returns the mapped reason; unrecognized values (content_filter, 鈥? become `{kind: 'error'}` with the uppercased value as `code`.
 */
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    // usage-only chunk 鈥?keep the latest.
